1. Genel API Notlar?
* Base URL (örnek): https://api.saglikburada.com/api/v1
* Format: application/json
* Auth:
o POST /auth/login ? JWT access token
o Sonraki tüm isteklerde: Authorization: Bearer {token}
* HTTP durum kodlar?:
o 200 OK
o 201 Created
o 400 Bad Request
...
2.3 POST /auth/login
Amaç:
Email & ?ifre ile giri? yapar, JWT access token üretir.
Request:
{
  "email": "user@example.com",
  "password": "P@ssw0rd!"
...
2.4 GET /users/me
Amaç:
Giri? yapm?? kullan?c?n?n temel user profilini döner.
Auth:
...
2.5 PUT /users/me
Amaç:
Kullan?c?n?n kendi User profilindeki temel alanlar? günceller (email hariç).
Request:
{
  "phone": "+905009998877"
}
...
3.1 GET /clients/me
Amaç:
Giri? yapm?? kullan?c?n?n Client detay kayd?n? döner.
Response (200 OK):
{
  "id": 45,
  "userId": 123,
...
3.2 PUT /clients/me
Amaç:
Dan??an?n kendi profil bilgilerini güncellemesi.
Request:
{
  "firstName": "Ahmet",
  "lastName": "Y?lmaz",
...
Amaç:
Admin’in belirli bir Client kayd?n? görüntülemesi.
Yetki:
...
4.1 GET /experts/me
Amaç:
Expert rolüne sahip kullan?c?n?n kendi uzman profilini döner.
Response:
{
  "id": 70,
  "userId": 200,
...
4.2 PUT /experts/me
Amaç:
Expert’in kendi profilini güncellemesi (bio, city, workType, experienceYear vb.).
Request:
{
  "displayName": "Dyt. Ay?e Kaya",
  "bio": "Online ve yüz yüze klinik diyetisyen...",
  "city": "Istanbul",
  "workType": "Hybrid",
  "experienceYear": 6
}
Response (200 OK): Güncellenmi? ayn? model.

4.3 GET /experts/{id}
Amaç:
Client taraf?nda uzman profil sayfas?n? göstermek için kullan?l?r.
* Public (login yok): s?n?rl? alanlar
* Login olmu? client: daha detayl?
Response (örnek – login olmu? client için):
{
  "id": 70,
  "expertType": "Dietitian",
  "displayName": "Dyt. Ay?e Kaya",
...
4.4 GET /experts
Amaç:
“Diyetisyenler / Uzmanlar” listeleme ekran? için kullan?l?r.
Query parametre örnekleri:
* expertType=Dietitian
* city=Istanbul
* specializationId=1
...
* Pending, Rejected, Passive olanlar listede görünmez.
...
Amaç:
Onboarding ekran? ve filtreler için uzmanl?k listesi.
